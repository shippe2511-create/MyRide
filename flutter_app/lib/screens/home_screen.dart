...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  void _showScheduledRideActive(Map<String, dynamic> ride) {
    if (!mounted) return;
    HapticFeedback.heavyImpact();
      ...
      SnackBar(
        content: Row(
          children: [
            ...
            Expanded(child: Text('Your scheduled ride is now being sent to drivers!')),
          ],
        ),
  ...
  void _showScheduledRideAccepted(Map<String, dynamic> ride) {
    if (!mounted) return;
    HapticFeedback.heavyImpact();
    final driverName = ride['driver']?['profile']?['full_name'] ?? 'Driver';
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            const SizedBox(width: 10),
            Text('Driver Found!', style: TextStyle(color: context.textColor)),
          ],
        ),
        content: Text('$driverName accepted your scheduled ride!', style: TextStyle(color: context.mutedColor)),
        actions: [
          ElevatedButton(
            onPressed: () {
              Navigator.pop(ctx);
              setState(() {
                _scheduledTime = null;
                _scheduledRideId = null;
              });
              // Navigate to trip tracking
              Navigator.push(context, MaterialPageRoute(
                builder: (_) => TripTrackingScreen(tripData: {
                  'rideId': ride['id'],
